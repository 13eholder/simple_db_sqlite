...
//设置内部节点最大 cell值为 3，便于测试
const u_int32_t INTERNAL_NODE_MAX_NUM_KEYS = 3;
...
void internal_node_insert(Table* table,
                          u_int32_t parent_page_num,
                          u_int32_t child_page_num);
...
/*
   ...
   half of N into L and the upper half into R. Now N is empty. Add 〈L, K,R〉 in
   N, where K is the max key in L. Page N remains the root.
*/
void leaf_node_split_and_insert(Cursor* cursor, u_int32_t key, Row* value)
{
...
void create_new_root(Table* table, u_int32_t right_child_page_num)
{
    ...
    if (get_node_type(left_child) == NODE_INTERNAL) {
        void* child;
        for (u_int32_t i = 0; i < *internal_node_num_keys(left_child); i++) {
...
// 内部结点的子节点可能是内部结点 也可能是叶子节点
// 若是叶子节点，调用 leaf_node_find
// 若是内部结点，递归调用
Cursor* internal_node_find(Table* table, u_int32_t page_num, u_int32_t key)
{
    void* node            = get_page(table->pager, page_num);
    ...
    switch (get_node_type(child)) {
        case NODE_LEAF: return leaf_node_find(table, page_num, key);
        case NODE_INTERNAL: return internal_node_find(table, page_num, key);
    }
}
u_int32_t internal_node_find_child(void* node, u_int32_t key)
{
                                    ...
                                    u_int32_t child_page_num)
{
    ...
    //根据是否需要拆分根节点进行分支
    u_int32_t splitting_root = is_root_node(old_node);
    void* parent;
    void* new_node;
    if (splitting_root) {
    ...
    *internal_node_right_child(old_node) = INVALID_PAGE_NUM;
    for (u_int32_t i = INTERNAL_NODE_MAX_CELLS - 1;
    ...
    *internal_node_right_child(old_node) =
    ...
    u_int32_t destination_page_num =
